...
gta.sed(gta.roi.sources[0].name, loge_bins=gta.log_energies[::1], outfile='PSR_SED.fits', free_background=True, write_npy=True, make_plots=True, write_fits=True)

# Test for Lightcurve #
print('--- Test for Lightcurve ---')
gta.free_sources(free=False)
gta.print_model()

# Building target Lightcurve #
print('--Building Lightcurve of Target--')
gta.lightcurve(gta.roi.sources[0].name, nbins=15, free_background=True, write_fits=True, write_npy=True, make_plots=True)